...
const TracksItem = ({track, index, setTracks, tracks, editPlaylist, setSelectedPlaylist, selectedPlaylist}) => {
    ...
    return (
        ...
        <>
            ...
            <td scope="col">{selected ? 
            
            <div class="input-group input-group-sm mt-1">
            <input type="text" className="form-control" aria-label="Small" value={input} placeholder="Select genre" onKeyDown={handleKeydown} onChange={handleChange}></input>
            </div>
            ...
            : track.genre}</td>
            {
                ...
                {/* <input class="-input" type={'checkbox'} defaultChecked={selected ? true : false} value="" onClick={handleSelect}></input> */}
                <FontAwesomeIcon onClick={handleSelect} icon={faPenToSquare} style={{cursor: "pointer"}}/>
                </div>
                </td>
                <td scope="col">
                <div class=" d-flex justify-content-center align-items-center p-1 mt-1">
